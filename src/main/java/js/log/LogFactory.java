 ...
 * implementation found on run-time, provider is initialized with {@link DefaultLogProvider}. Provider instance is
 * delegated for most of this factory tasks.
 * 
 * <p>
 * Before to be used, a logging provider may require configuration. It is highly probable but still configuration step
 * is not mandatory. For configuration, log factory has {@link #config(Config)} that just passes configuration object to
 * provider. Configuration object structure is provider implementation detail. Provider is encourage to initialize
 * itself with sensible default configuration in case user code fails to invoke {@link #config(Config)}.
 * 
 * <p>
 ...
 * {@link #getLog(Class)}. In fact this is most common usage pattern: every class has its own logger instance, see
 * sample code.
 * 
 * <pre>
 * public class Sample {
...
public final class LogFactory
{
  ...
  /**
   * Load log provider from Java services and return the first instance found. It is expected to have only one log
   * service provider deployed on runtime; it more found just blindly select the first found.
   * <p>
  ...
  /**
   * Configure logging provider from configuration object. Configuration object format is entirely under provider
   * control; this method just pass it as it is.
   * <p>
